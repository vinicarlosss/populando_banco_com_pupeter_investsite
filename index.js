...
console.table(["Adicionar dados", "Atualizar dados"])
    ...
    if(escolha == 0){
        ...
        (async()=>{
            for(let i = 0; i<EMPRESAS.length;i++){
                dados = await webScraping(EMPRESAS[i][0])
                db.insertEmpresa(EMPRESAS[i][0], EMPRESAS[i][1],dados.ebit,dados.enterprise_value)
                console.log(i+1 +` Empresa: ${EMPRESAS[i][0]}`)
            }
        })();
    }else if(escolha == 1){
        let dados
        (async()=>{
            for(let i = 0; i<EMPRESAS.length;i++){
                dados = await webScraping(EMPRESAS[i][0])
                db.updateEmpresa(EMPRESAS[i][0], EMPRESAS[i][1],dados.ebit,dados.enterprise_value)
                console.log(i+1 +` Empresa: ${EMPRESAS[i][0]}`)
            }
        })();
    }
